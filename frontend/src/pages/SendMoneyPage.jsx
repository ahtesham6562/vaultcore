            ...
            <div style={styles.card}>
                ...
                {step === 1 && (
                    <div style={styles.body}>
                        ...
                        <input style={styles.input} type="text" name="toAccountNo"
                            placeholder="Receiver Account No (e.g. ACC-00482)"
                            value={form.toAccountNo} onChange={handleChange} />
                        <input style={styles.input} type="number" name="amount"
                            placeholder="Amount (₹)" value={form.amount}
                ...
                {step === 2 && (
                    <div style={styles.body}>
                        ...
                        <div style={styles.btnRow}>
                            <button style={styles.btnOutline} onClick={() => setStep(1)}>← Edit</button>
                ...
                {step === 3 && result && (
                    <div style={styles.body}>
                        {result.fraudFlagged ? (
                            <>
                                <div style={{...styles.successIcon, background: '#ff6b6b'}}>⚠️</div>
                                <p style={styles.successTitle}>Transaction Flagged!</p>
                                <p style={{color: '#888', textAlign: 'center', fontSize: '13px', margin: 0}}>
                                    Amount ₹{result.amount} exceeds fraud threshold. 2FA required.
                                </p>
                                <div style={styles.confirmBox}>
                                    <div style={styles.confirmRow}>
                                    ...
                                    <div style={styles.confirmRow}>
                                        ...
                                        <span style={{...styles.confirmValue, color: '#ff6b6b'}}>PENDING_2FA</span>
                                    </div>
                                </div>
                            </>
                        ) : (
                        ...
                        <button style={styles.btn} onClick={onBack}>Back to Dashboard</button>
                    </div>
                )}
            </div>
        </div>
...
const styles = {
    container: { minHeight: '100vh', background: 'linear-gradient(135deg, #0f0c29, #302b63, #24243e)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'sans-serif' },
    card: { background: 'rgba(255,255,255,0.05)', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px', width: '420px', overflow: 'hidden' },
    header: { padding: '20px 24px', borderBottom: '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' },
    ...
    btn: { padding: '12px', borderRadius: '8px', border: 'none', background: '#6c63ff', color: '#fff', fontSize: '15px', cursor: 'pointer', fontWeight: 'bold' },
    btnOutline: { padding: '12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.2)', background: 'transparent', color: '#fff', fontSize: '15px', cursor: 'pointer', flex: 1 },
    btnRow: { display: 'flex', gap: '10px' },
    error: { color: '#ff6b6b', fontSize: '13px', margin: 0 },
    confirmBox: { background: 'rgba(255,255,255,0.05)', borderRadius: '10px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' },
    ...
    successIcon: { width: '60px', height: '60px', borderRadius: '50%', background: '#4caf50', color: '#fff', fontSize: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto' },
